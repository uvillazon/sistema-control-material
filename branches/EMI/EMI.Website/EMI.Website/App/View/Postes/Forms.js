Ext.define("App.View.Postes.Forms", {
    ...
    eventosPoste: function (btn) {
        ...
        else if (btn.getItemId() == 'btn_QuitarCodSol') {
            var data = me.grid.getSelectionModel().getSelection()[0];
            me.cbx_codigoSolucion.setValue(data.get('ID_COD_SOL'));
            
            if (data != null) {
                Funciones.AjaxRequestForm("Codigos", "EliminarCodigoSolucionCodigoMantenimiento", me, me, me.grid, "¿Esta Seguro de Eliminar el registro?", { ID_COD_MAN: data.get('ID_COD_MAN'), ID_COD_SOL: data.get('ID_COD_SOL') }, null);
            } else {
                Ext.msg.alert("Error", "Seleccione el registro que desea eliminar");
            }
        }
